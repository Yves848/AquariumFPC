...
begin
  ...
  AResponse.Contents.Text :=
    ...
    '  <title>Dashboard Aquarium</title>' + LineEnding +
    '  <style>' + LineEnding +
    '    body { font-family: sans-serif; background-color: #f2f2f2; padding: 2em; }' + LineEnding +
    '    h1 { color: #2c3e50; }' + LineEnding +
    '    .card { background: white; padding: 1em; border-radius: 10px; box-shadow: 2px 2px 8px rgba(0,0,0,0.1); width: 400px; }' + LineEnding +
    '    .label { font-weight: bold; }' + LineEnding +
    '  </style>' + LineEnding +
    '</head>' + LineEnding +
    '<body>' + LineEnding +
    '<h1>Tableau de bord de l''aquarium</h1>' + LineEnding +
    '<div class="card">' + LineEnding +
    Format('<p><span class="label">Mode :</span> %s</p>', [IfThen(CurrentServiceMode = smAuto, 'Automatique', 'Manuel')]) + LineEnding +
    Format('<p><span class="label">État manuel :</span> %s</p>', [ManualState]) + LineEnding +
    Format('<p><span class="label">Heure d''allumage :</span> %s</p>', [OnTime]) + LineEnding +
    Format('<p><span class="label">Heure d''extinction :</span> %s</p>', [OffTime]) + LineEnding +
    Format('<p><span class="label">Dernière commande envoyée :</span> %s</p>', [LastCommand]) + LineEnding +
    '</div>' + LineEnding +
    '</body>' + LineEnding +
    '</html>';
  AResponse.Code := 200;
